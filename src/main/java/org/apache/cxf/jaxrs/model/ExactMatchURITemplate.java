...
public final class ExactMatchURITemplate {
    ...
    public ExactMatchURITemplate(String theTemplate) {
...
//        patternBuilder.append(LIMITED_REGEX_SUFFIX);
    ...
    public static ExactMatchURITemplate createTemplate(Path path) {

        return createTemplate(path == null ? null : path.value());
    }

    public static ExactMatchURITemplate createTemplate(String pathValue) {

        if (pathValue == null) {
            return new ExactMatchURITemplate("/");
        }
        ...
        return new ExactMatchURITemplate(pathValue);
    }

    public static int compareTemplates(ExactMatchURITemplate t1, ExactMatchURITemplate t2) {
     ...
     * segments can have multiple chunks. Chunk is used to decompose URI of {@link ExactMatchURITemplate} into literals
    ...
    /**
     * Variable of ModifiedURITemplate. Variable has either "{varname:pattern}" syntax or "{varname}".
     */
     ...
     * "{bar}" and "baz}blah".
     * <p>
     * This is helper class for {@link ExactMatchURITemplate} that enables recurring literals appearing next to regular
